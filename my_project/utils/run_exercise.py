...
class ExerciseTopo(Topo):
    ...
    def __init__(self, hosts, switches, links, log_dir, bmv2_exe, pcap_dir, **opts):
        ...
        for sw, params in switches.items():
            ...
            # Extract CMCS-specific switch parameters from topology.json and
            # forward them to the switch constructor:
            #   - "vn_queues"  : number of VNs sharing the physical link on
            #                    this switch; determines how many per-VN FIFO
            #                    queues QueueingLogicVN allocates.
            #   - "vn_weights" : list of sharing weights [w_1, w_2, ..., w_k]
            #                    used by the HRDS Phase-2 Deficit Weighted
            #                    Round Robin to distribute residual link
            #                    capacity among VNs with excess demand.
            for key in ("cpu_port", "vn_queues", "vn_weights"):
                if key in params:
...
class ExerciseRunner:
    ...
    def __init__(self, topo_file, log_dir, pcap_dir,
        ...
        """ Initializes some attributes and reads the topology json. Does not
            ...
            Arguments:
                ...
                test  : bool          // If True, run the automated throughput test
                                         instead of dropping into the interactive
                                         Mininet CLI (triggered by --test / make test).
        """
        ...
        # Retain the test flag so that do_net_cli() can choose between
        # automated experiment mode and interactive CLI mode.
        self.test = test
    ...
    def do_net_cli(self):
        """ Starts up the mininet CLI and prints some helpful output.
            ...
            If the --test flag was passed at startup, this method invokes the
            automated throughput experiment (run_experiment) from the test/
            subdirectory instead of opening the interactive Mininet CLI.  This
            allows 'make test' to drive the full CMCS validation workflow
        ...
        if self.test:
            # Automated test mode (--test / make test):
            # Import run_experiment from the test/ subdirectory and execute it
            # directly against the running Mininet instance.  The test/
            # directory is inserted at the front of sys.path so that the import
            ...
            run_experiment(self.net)
        else:
            # Interactive mode (make run): drop into the standard Mininet CLI.
            CLI(self.net)
...
def get_args():
    ...
    # When present, this flag causes do_net_cli() to run the automated
    # throughput experiment rather than opening the interactive Mininet CLI.
    # Intended to be set by 'make test' in the project Makefile.
    parser.add_argument('--test', action='store_true',
                        help='Run automated throughput test instead of CLI')
