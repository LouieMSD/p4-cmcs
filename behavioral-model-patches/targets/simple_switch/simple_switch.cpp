...
// CMCS modification: the constructor signature accepts vn_weights
// (std::vector<int>) in place of the original nb_queues_per_port (size_t).
// nb_vns is derived from vn_weights.size() and cached for O(1) bounds checks
// in the enqueue fast path.  egress_buffers (QueueingLogicVN) is initialised
// with the same vector so that it creates the correct number of per-VN FIFO
// queues and configures the DWRR sharing weights w_1 ... w_k from the start.
...
SimpleSwitch::~SimpleSwitch() {
  ...
  for (size_t i = 0; i < nb_egress_threads; i++) {
    // A null sentinel packet (packet == nullptr) is pushed into each egress
    // worker's queue to signal it to terminate.  color is set to 0 as a
    // placeholder; the egress thread breaks out of its loop immediately upon
    // seeing a null packet and never reads the color value.  The push is
    // retried in a loop because the queue may be temporarily full due to
    // in-flight egress clones.
...
// Set the maximum queue depth for VN vn_id on egress port port.
// Delegates directly to QueueingLogicVN::set_vn_capacity.
int
SimpleSwitch::set_egress_vn_queue_depth(size_t port, size_t vn_id,
                                           const size_t depth_pkts) {
  egress_buffers.set_vn_capacity(port, vn_id, depth_pkts);
  return 0;
}

// Set the queue depth for all VNs on egress port port.
int
SimpleSwitch::set_egress_queue_depth(size_t port, const size_t depth_pkts) {
  egress_buffers.set_queue_capacity(port, depth_pkts);
  return 0;
}

// Set the queue depth for all VNs on all egress ports.
int
...
// Set the dequeue rate limit for egress port port.
// Rate limiting is enforced at the per-port granularity by the leaky bucket
// inside QueueingLogicVN, modelling the physical link capacity constraint.
int
...
// Set the dequeue rate limit for all egress ports.
int
...
SimpleSwitch::enqueue(port_t egress_port, std::unique_ptr<Packet> &&packet) {
    ...
    // Read the VN id written by the Ingress Pipeline color marker into
    // meta.vn_id.  The Traffic Manager uses this value to route the packet
    // into the correct per-VN FIFO queue under the target egress port.
    // If the field is absent (e.g. the P4 program does not define it), vn_id
    // defaults to 0, directing all traffic to VN 0.
    size_t vn_id = phv->has_field(SSWITCH_VN_QUEUEING_SRC) ?
        phv->get_field(SSWITCH_VN_QUEUEING_SRC).get<size_t>() : 0u;

    // Guard against an out-of-range vn_id produced by the data plane.
    // Rather than dropping the packet, clamp to VN 0 to preserve connectivity
    // and emit a warning so the misconfiguration is visible in the logs.
    if (vn_id >= nb_vns) {
      bm::Logger::get()->warn(
          "vn_id {} out of range (nb_vns={}), clamping to 0",
          vn_id, nb_vns);
      vn_id = 0u;
    }

    // Read the color value written by the Ingress Pipeline color marker into
    // meta.color.  The Traffic Manager uses this value to decide whether to
    // increment the Guaranteed Byte Credit (GB credit) counter for the VN
    // queue: a Green packet (color == 0) triggers a credit increment equal to
    // the packet size; a Yellow packet (color == 1) does not.
    // If the field is absent, color defaults to 0 (Green).
    size_t color = phv->has_field(SSWITCH_COLOR_SRC) ?
        phv->get_field(SSWITCH_COLOR_SRC).get<size_t>() : 0u;

    // Record the enqueue timestamp and current VN queue depth into
    // queueing_metadata so that the Egress Pipeline can observe queuing delay
    // and instantaneous queue occupancy.  This is independent of the CMCS
    // scheduling mechanism and requires the P4 program to declare the standard
    // queueing_metadata header.
    if (with_queueing_metadata) {
      phv->get_field("queueing_metadata.enq_timestamp").set(get_ts().count());
      phv->get_field("queueing_metadata.enq_qdepth")
          .set(egress_buffers.size(egress_port, vn_id));
    }

    // Hand the packet off to the Traffic Manager.  Ownership is transferred
    // via std::move; the packet will be held in the per-VN FIFO deque of the
    // target egress port until the CMCS scheduler selects it for transmission.
    egress_buffers.push_front(
        egress_port, vn_id, color,
        std::move(packet));
}
...
SimpleSwitch::egress_thread(size_t worker_id) {
  ...
  while (1) {
    std::unique_ptr<Packet> packet;
    size_t port;
    // Receive the next packet selected by the CMCS scheduler along with the
    // egress port, VN id, and color of the dequeued packet.  The scheduler
    // runs the two-phase algorithm (GB-credit priority service followed by
    // DWRR residual sharing) inside pop_back and blocks this thread until a
    // packet is available and the per-port dequeue leaky bucket allows it to
    // leave.
    size_t vn_id;
    size_t color;
    egress_buffers.pop_back(worker_id, &port, &vn_id, &color, &packet);
    if (packet == nullptr) break;
    ...
    if (with_queueing_metadata) {
      ...
      // Report the current VN queue depth at dequeue time so the Egress
      // Pipeline can observe instantaneous queue occupancy per VN.
      phv->get_field("queueing_metadata.deq_qdepth").set(
          egress_buffers.size(port, vn_id));
      // Write the VN id back into queueing_metadata.qid so the Egress
      // Pipeline can identify which VN this packet was served from, enabling
      // per-VN telemetry and statistics collection.
      if (phv->has_field("queueing_metadata.qid")) {
        auto &qid_f = phv->get_field("queueing_metadata.qid");
        qid_f.set(vn_id);
      }
      // Restore the color value into the PHV so the Egress Pipeline sees the
      // same green/yellow marking that was assigned by the Ingress Pipeline
      // color marker.  The color is preserved across the queuing stage via the
      // QE struct and written back here to guard against any PHV reset or clone
      // operation that might have cleared it while the packet was buffered.
      if (phv->has_field(SSWITCH_COLOR_SRC)) {
          phv->get_field(SSWITCH_COLOR_SRC).set(color);
      }
    }
