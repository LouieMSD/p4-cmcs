...
class SimpleSwitchHandler : virtual public SimpleSwitchIf {
 public:
  ...
  // CMCS modification: the Thrift interface retains the original
  // set_egress_priority_queue_depth signature for backward compatibility with
  // existing control-plane clients.  The priority parameter is reinterpreted
  // as vn_id and forwarded to set_egress_vn_queue_depth, which sets the
  // packet capacity of the specified per-VN FIFO queue under the given port.
  int32_t set_egress_priority_queue_depth(const int32_t port_num,
    ...
    return switch_->set_egress_vn_queue_depth(
  ...
  // CMCS modification: per-VN rate control has been removed; the dequeue rate
  // limit is now applied at the logical-queue (egress port) level by the bps
  // leaky bucket inside QueueingLogicVN.  This Thrift handler is kept as a
  // no-op stub so that existing control-plane clients that call
  // set_egress_priority_queue_rate do not encounter a missing symbol error.
  int32_t set_egress_priority_queue_rate(const int32_t port_num,
                                         const int32_t priority,
                                         const int64_t rate_pps) {
    bm::Logger::get()->trace("set_egress_priority_queue_rate [no-op: per-VN rate removed]");
    (void)port_num; (void)priority; (void)rate_pps;
    return 0;
  }
