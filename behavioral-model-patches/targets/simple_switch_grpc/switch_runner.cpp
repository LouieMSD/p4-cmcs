...
SimpleSwitchGrpcRunner::SimpleSwitchGrpcRunner(
    ...
    // Receives the per-VN sharing weights parsed from --slice-weights on the
    // command line. The vector length determines the total number of VNs, and
    // each element vn_weights[i] specifies the sharing weight w_i used by the
    // Hybrid RR-DWRR Scheduler to distribute residual link capacity among VNs
    // that exceed their guaranteed bandwidth.
    ...
    // Forward vn_weights to the SimpleSwitch constructor so that the Traffic
    // Manager can initialize one dedicated FIFO queue per VN and configure the
    // per-VN sharing weights before any packet arrives.
